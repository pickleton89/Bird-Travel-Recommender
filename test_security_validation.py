...
from bird_travel_recommender.utils.prompt_sanitizer import PromptSanitizer, sanitize_for_birding_advice
...
def test_birding_advice_sanitization():
    ...
    if not result['is_safe']:
        print(f"✅ Malicious birding query correctly detected: {result['threats_detected']}")
    else:

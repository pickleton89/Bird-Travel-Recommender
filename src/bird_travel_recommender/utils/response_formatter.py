...
class EnhancedResponseFormatter:
    ...
    def format_response(
        self,
        tool_results: Union[Dict, List[Dict]],
        ...
        try:
            ...
            if isinstance(tool_results, list):
                ...
                formatted_response = self._format_multi_tool_results(
                    tool_results, formatting_context
                )
            else:
                # Single tool result
                formatted_response = self._format_single_tool_result(
                    tool_results, formatting_context
                )
            ...
            logger.info(
                f"Response formatted successfully: {formatting_context.response_type.value}"
            )
            ...
            return self._create_fallback_response(
                tool_results, formatting_context, str(e)
            )

    def _format_single_tool_result(
        self, tool_result: Dict[str, Any], context: FormattingContext
    ) -> Dict[str, Any]:
    ...
    def _format_complete_trip_response(
        self, result_data: Dict[str, Any], context: FormattingContext
    ) -> Dict[str, Any]:
    ...
    def _format_sightings_response(
        self, result_data: Dict[str, Any], context: FormattingContext
    ) -> Dict[str, Any]:
        ...
        species_summary = self._summarize_sightings_by_species(
            sightings, context.extracted_species
        )
        ...
        response = {
            ...
            "best_opportunities": self._identify_best_sighting_opportunities(
                sightings, context
            ),
    ...
    def _enhance_itinerary_with_llm(
        self, base_itinerary: str, context: FormattingContext
    ) -> str:
...
Format as clean markdown with clear sections, bullet points, and practical advice. Keep the enthusiastic but professional tone of an experienced guide."""
    ...
    def _adapt_advice_to_user_level(
        self, advice: str, context: FormattingContext
    ) -> str:
...
Maintain accuracy while making the content engaging and accessible."""
    ...
    def _generate_trip_summary(
        self, trip_plan: Dict[str, Any], context: FormattingContext
    ) -> str:
        """Generate a compelling trip summary"""

        duration = trip_plan.get("trip_duration_days", 1)
        species_count = (
            len(context.extracted_species) if context.extracted_species else "several"
        )
        ...
        species_text = (
            ...
            if isinstance(species_count, int)
            else f"{species_count} target species"
        )
        location_text = f"{location_count} location" + (
            "s" if location_count != 1 else ""
        )
    ...
    def _format_location_details(
        self, locations: List[Dict], context: FormattingContext
    ) -> List[Dict[str, Any]]:
    ...
    def _summarize_sightings_by_location(self, sightings: List[Dict]) -> Dict[str, Any]:
        ...
        for sighting in sightings:
            ...
            location_groups[loc_name]["species_list"].append(
                sighting.get("comName", "Unknown")
            )
            location_groups[loc_name]["total_observations"] += (
                sighting.get("howMany", 1)
                if isinstance(sighting.get("howMany"), int)
                else 1
            )
            ...
            if obs_date and (
                not location_groups[loc_name]["most_recent"]
                or obs_date > location_groups[loc_name]["most_recent"]
            ):
                location_groups[loc_name]["most_recent"] = obs_date

        # Sort by species diversity
        sorted_locations = sorted(
            location_groups.values(), key=lambda x: x["species_count"], reverse=True
        )
    ...
    def _create_fallback_response(
        self, tool_results: Union[Dict, List], context: FormattingContext, error: str
    ) -> Dict[str, Any]:
...
{data_notes}
    ...
    def _assess_location_accessibility(
        self, location: Dict, context: FormattingContext
    ) -> str:
    ...
    def _suggest_best_visiting_time(
        self, location: Dict, context: FormattingContext
    ) -> str:
    ...
    def _extract_location_highlights(
        self, location: Dict, context: FormattingContext
    ) -> List[str]:
    ...
    def _format_route_summary(
        self, route: Dict, context: FormattingContext
    ) -> Dict[str, Any]:
    ...
    def _generate_relevant_resources(
        self, advice_type: str, context: FormattingContext
    ) -> List[str]:
    ...
    def _generate_sightings_title(self, context: FormattingContext) -> str:
        ...
        species_part = (
            f" for {', '.join(context.extracted_species)}"
            if context.extracted_species
            else ""
        )
        location_part = (
            f" near {', '.join(context.extracted_locations)}"
            if context.extracted_locations
            else ""
        )
        return f"Recent Bird Sightings{species_part}{location_part}"

    def _generate_sightings_summary(
        self, statistics: Dict, context: FormattingContext
    ) -> str:
    ...
    def _summarize_sightings_by_species(
        self, sightings: List[Dict], target_species: List[str]
    ) -> Dict[str, Any]:
        ...
        for sighting in sightings:
            ...
            species_groups[species]["locations"].append(
                sighting.get("locName", "Unknown")
            )

        return {"species_summary": species_groups, "total_species": len(species_groups)}
    ...
    def _identify_best_sighting_opportunities(
        self, sightings: List[Dict], context: FormattingContext
    ) -> List[str]:
    ...
    def _generate_data_interpretation_notes(
        self, statistics: Dict, context: FormattingContext
    ) -> List[str]:
    ...
    def _format_species_validation_response(
        self, result_data: Dict[str, Any], context: FormattingContext
    ) -> Dict[str, Any]:
    ...
    def _format_location_scores_response(
        self, result_data: Dict[str, Any], context: FormattingContext
    ) -> Dict[str, Any]:

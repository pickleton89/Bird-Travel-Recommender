...
class PromptSanitizer:
    ...
    INJECTION_PATTERNS = [
        ...
        (
            r"(?:new|different|updated)\s+(?:instructions?|commands?|system\s+prompt)",
        ...
        (
            r"(?:you\s+are|act\s+as|pretend\s+to\s+be|roleplay\s+as)\s+(?:a\s+)?(?:different|new|evil|malicious)",
            "role_hijack",
        ),
        ...
        (
            r"in\s+a\s+(?:fictional|fantasy|alternate)\s+(?:world|universe|scenario)",
            "fictional_jailbreak",
        ),
    ...
    MAX_LENGTHS = {"query": 1000, "species_name": 100, "location": 200, "general": 500}

    @classmethod
    def sanitize_prompt_input(
        cls, text: str, input_type: str = "general", strict_mode: bool = True
    ) -> SanitizationResult:
        """
        ...
        for pattern, threat_type in cls.INJECTION_PATTERNS:
            if re.search(pattern, text, re.IGNORECASE | re.MULTILINE):
                ...
                if strict_mode:
                    ...
                    text = re.sub(
                        pattern, "[FILTERED]", text, flags=re.IGNORECASE | re.MULTILINE
                    )
    ...
    def create_safe_prompt(
        cls, template: str, user_inputs: Dict[str, str], strict_mode: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        for key, value in user_inputs.items():
            result = cls.sanitize_prompt_input(
                value, input_type=key, strict_mode=strict_mode
            )
...
Provide actionable advice that will help the birder succeed."""
    ...
    user_inputs = {"query": query, "context_info": context_info}
...
Provide the most likely match with confidence level."""
    ...
    user_inputs = {"species_name": species_name}

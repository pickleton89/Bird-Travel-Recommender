...
class EBirdAsyncClient(
    ...
    async def batch_recent_observations(
        ...
        """
        logger.info(
            f"Fetching recent observations for {len(region_codes)} regions concurrently"
        )
        ...
        logger.info(
            f"Batch operation completed: {total_observations} total observations from {len(region_codes)} regions"
        )

        return batch_results

    async def batch_nearby_hotspots(
        self, coordinates: List[tuple], distance_km: int = 25
    ) -> Dict[str, List[Dict[str, Any]]]:
        """
        ...
        """
        logger.info(
            f"Fetching nearby hotspots for {len(coordinates)} locations concurrently"
        )
        ...
        logger.info(
            f"Batch hotspot operation completed: {total_hotspots} total hotspots from {len(coordinates)} locations"
        )

        return batch_results

    async def batch_species_validation(
        self, species_codes: List[str], batch_size: int = 50
        ...
        """
        logger.info(
            f"Validating {len(species_codes)} species codes in batches of {batch_size}"
        )

        # Split into batches
        batches = [
            species_codes[i : i + batch_size]
        ...
        logger.info(
            f"Species validation completed: {len(all_validated)} species validated"
        )
    ...
    async def test_async_client():
        try:
            async with EBirdAsyncClient() as client:
                ...
                batch_results = await client.batch_recent_observations(
                    region_codes, days_back=3
                )

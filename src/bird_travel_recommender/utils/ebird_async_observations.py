...
class EBirdAsyncObservationsMixin:
    ...
    async def get_nearby_observations(
        ...
        try:
            result = await self.make_request(endpoint, params)
            logger.info(
                f"Retrieved {len(result)} nearby observations within {distance_km}km of ({lat}, {lng})"
            )
            return result
        except EBirdAPIError as e:
    ...
    async def get_notable_observations(
        self, region_code: str, days_back: int = 7, include_provisional: bool = False
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def get_nearest_observations(
        ...
        try:
            result = await self.make_request(endpoint, params)
            logger.info(
                f"Retrieved {len(result)} nearest {species_code} observations near ({lat}, {lng})"
            )
            return result
        except EBirdAPIError as e:
            logger.error(
                f"Error fetching nearest {species_code} observations for ({lat}, {lng}): {e}"
            )
            raise
    ...
    async def get_nearby_notable_observations(
        ...
        try:
            result = await self.make_request(endpoint, params)
            logger.info(
                f"Retrieved {len(result)} nearby notable observations within {distance_km}km of ({lat}, {lng})"
            )
            return result
        except EBirdAPIError as e:
            logger.error(
                f"Error fetching nearby notable observations for ({lat}, {lng}): {e}"
            )
            raise
    ...
    async def get_nearby_species_observations(
        ...
        try:
            result = await self.make_request(endpoint, params)
            logger.info(
                f"Retrieved {len(result)} nearby {species_code} observations within {distance_km}km of ({lat}, {lng})"
            )
            return result
        except EBirdAPIError as e:
            logger.error(
                f"Error fetching nearby {species_code} observations for ({lat}, {lng}): {e}"
            )
            raise

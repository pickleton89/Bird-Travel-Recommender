"""
Async eBird API client for historical analysis and trend-related endpoints.

This module provides async methods for analyzing historical bird data from the eBird API,
...
class EBirdAsyncAnalysisMixin:
    """Async mixin class providing historical analysis and trend-related eBird API methods."""
    ...
    async def get_historic_observations(
        ...
        try:
            observations = await self.make_request(endpoint, params)
            ...
            for month in key_months:
                ...
                for year in range(start_year, end_year + 1):
                    try:
                        ...
                        sample_obs = await self.get_historic_observations(
                            region=region,
                            year=year,
                            month=month,
                            day=15,
    ...
    async def get_yearly_comparisons(
        self,
        region: str,
        reference_date: str,
        ...
        try:
            ...
            for year in years_to_compare:
                try:
                    observations = await self.get_historic_observations(
                        region=region,
                        year=year,
                        month=month,
                        day=day,
        ...
        try:
            ...
            for month in months:
                try:
                    ...
                    month_observations = await self.get_species_observations(
                        species_code=species_code,
    ...
    async def get_peak_times(
        ...
        try:
            ...
            recent_observations = await self.get_nearby_species_observations(
                species_code=species_code,
                lat=lat,
                lng=lng,
...
class EBirdAsyncAnalysisClient(EBirdAsyncBaseClient, EBirdAsyncAnalysisMixin):
    """Complete async eBird API client for historical analysis and trend-related endpoints."""
    pass
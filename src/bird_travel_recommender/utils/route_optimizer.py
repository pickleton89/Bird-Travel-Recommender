...
def optimize_birding_route(
    ...
    if len(locations) == 1:
        return {
            ...
            "total_distance_km": haversine_distance(
                start_location["lat"],
                start_location["lng"],
                locations[0]["center_lat"],
                locations[0]["center_lng"],
            )
            * 2,  # Round trip
    ...
    if len(locations) > max_locations:
        ...
        sorted_locations = sorted(
            locations, key=lambda loc: loc.get("final_score", 0), reverse=True
        )
        locations_to_optimize = sorted_locations[:max_locations]
        logger.info(
            f"Limited optimization to top {max_locations} locations (from {len(locations)})"
        )
    else:
    ...
    if len(locations_to_optimize) <= 8:
        ...
        return _optimize_large_route_nearest_neighbor(
            start_location, locations_to_optimize
        )


def _optimize_small_route(
    start_location: Dict[str, float], locations: List[Dict[str, Any]]
) -> Dict[str, Any]:
    """
    ...
    return {
        ...
        "optimization_stats": {
            ...
            "improvement_pct": (
                (nn_result["total_distance"] - total_distance)
...
def _optimize_large_route_nearest_neighbor(
    start_location: Dict[str, float], locations: List[Dict[str, Any]]
) -> Dict[str, Any]:
    """
    ...
    for start_candidate in start_candidates:
        if start_candidate is None:
            ...
            partial_result = _nearest_neighbor_route_from_location(
                start_candidate, remaining_locations
            )
            ...
            first_distance = haversine_distance(
                start_location["lat"],
                start_location["lng"],
                start_candidate["center_lat"],
                start_candidate["center_lng"],
            )
            last_location = (
                partial_result["route"][-1]
                if partial_result["route"]
                else start_candidate
            )
            return_distance = haversine_distance(
                last_location["center_lat"],
                last_location["center_lng"],
                start_location["lat"],
                start_location["lng"],
            )
...
def _nearest_neighbor_route(
    ...
    while remaining:
        ...
        current_lat, current_lng = (
            nearest_location["center_lat"],
            nearest_location["center_lng"],
        )
...
def _nearest_neighbor_route_from_location(
    start_location: Dict[str, Any], remaining_locations: List[Dict[str, Any]]
) -> Dict[str, Any]:
    """
    ...
    current_lat, current_lng = (
        start_location["center_lat"],
        start_location["center_lng"],
    )
    total_distance = 0.0
...
def _two_opt_improvement(
    start_location: Dict[str, float], route: List[Dict[str, Any]]
) -> List[Dict[str, Any]]:
    """
...
def calculate_route_segments(
    ...
    for i, location in enumerate(route):
        ...
        segment = {
            ...
            "cumulative_distance_km": sum(seg.get("distance_km", 0) for seg in segments)
            + distance,
            "location_score": location.get("final_score", 0),
            "species_diversity": location.get("statistics", {}).get(
                "species_diversity", 0
            ),
        }
        ...
        segments.append(
            {
                ...
                "cumulative_distance_km": segments[-1]["cumulative_distance_km"]
                + return_distance,
                "location_score": 0,
                "species_diversity": 0,
            }

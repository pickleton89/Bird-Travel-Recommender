...
class EBirdAsyncTaxonomyMixin:
    ...
    async def get_taxonomy(
        ...
        params = {"fmt": format, "locale": locale}
        ...
        try:
            ...
            if species_codes:
                logger.info(
                    f"Retrieved taxonomy for {len(species_codes)} species codes"
                )
            else:
                logger.info(
                    f"Retrieved complete eBird taxonomy ({len(result)} entries)"
                )
            return result
    ...
    async def get_species_list(
        self, region_code: str, include_provisional: bool = False
    ) -> List[str]:
        """
        ...
        try:
            result = await self.make_request(endpoint, params)
            logger.info(
                f"Retrieved species list for {region_code}: {len(result)} species"
            )
            return result
        except EBirdAPIError as e:
    ...
    async def validate_species_codes(
        ...
        try:
            ...
            logger.info(
                f"Validated {len(validated)} of {len(species_codes)} species codes"
            )
            return validated
    ...
    async def search_species_by_name(
        self, common_name: str, region_code: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def get_regional_species_stats(self, region_code: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"Generated species stats for {region_code}: {stats['total_species']} species"
            )
            return stats

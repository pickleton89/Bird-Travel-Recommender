...
Example usage:
  ...
  python main.py --output my_itinerary.md
    ...
    parser.add_argument(
        "--output", "-o", 
        help="Output file for generated itinerary (markdown format)"
    )
    parser.add_argument(
        "--debug", "-d",
        action="store_true",
    ...
    parser.add_argument(
        "--test",
        action="store_true", 
        help="Run with test data (default behavior)"
    )
    ...
    print(f"Starting Location: ({start_location.get('lat', 'unknown')}, {start_location.get('lng', 'unknown')})")
    print(f"Region: {constraints.get('region', 'Not specified')}")
    print(f"Max Distance: {constraints.get('max_daily_distance_km', 'Not specified')} km")
    print("="*60)
    print()
    ...
    try:
        ...
        if result["success"]:
            ...
            print(f"   Route distance: {stats.get('route_optimization_stats', {}).get('total_route_distance_km', 0):.1f} km")
            print(f"   Itinerary method: {stats.get('itinerary_generation_stats', {}).get('itinerary_method', 'unknown')}")

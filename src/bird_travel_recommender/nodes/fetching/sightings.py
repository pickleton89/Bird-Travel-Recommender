...
class FetchSightingsNode(BatchNode):
    ...
    def exec(self, species: Dict[str, Any]) -> Dict[str, Any]:
        ...
        task = self._create_fetch_task(
            species, region_code, days_back, start_location, max_distance_km
        )
        ...
        try:
            ...
            logger.error(
                f"Unexpected error fetching sightings for {species['common_name']}: {e}"
            )
            return {
                "success": False,
                "error": str(e),
    ...
    def _create_fetch_task(
        ...
        """
        task = {"species": species, "region_code": region_code, "days_back": days_back}
        ...
        if start_location and start_location.get("lat") and start_location.get("lng"):
            ...
            task["distance_km"] = min(
                max_distance_km // 2, 50
            )  # Half max distance, capped at eBird limit
        else:
    ...
    def _fetch_species_sightings(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(
                f"Fetched {len(enriched_sightings)} sightings for {species['common_name']} using {method}"
            )
    ...
    def post(self, shared, prep_res, exec_res):
        ...
        if len(prep_res) == 0:
            ...
            logger.info(
                f"Sightings fetch completed: {success_rate:.1%} success rate, "

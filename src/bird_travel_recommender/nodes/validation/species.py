...
class ValidateSpeciesNode(Node):
    ...
    def exec(self, species_list: List[str]) -> Dict[str, Any]:
        ...
        logger.info(
            f"Validated {len(validated_species)} of {len(species_list)} species"
        )
    ...
    def _direct_taxonomy_lookup(
        self, species_name: str, taxonomy: List[Dict]
        ...
        for species in taxonomy:
            ...
            if (
                normalized_input in common_name and len(normalized_input) > 3
...
"""
    ...
    def _llm_only_validation(
        self, species_list: List[str], stats: Dict
    ) -> Dict[str, Any]:
        """
...
"""
        ...
        return {"validated_species": validated_species, "processing_stats": stats}

    def _format_validated_species(
        self, species: Dict, original_name: str, method: str, confidence: float
    ) -> Dict[str, Any]:
        """
    ...
    def _format_taxonomy_for_llm(self, taxonomy_subset: List[Dict]) -> str:
        ...
        for species in taxonomy_subset[:20]:  # Limit to avoid token overuse
            formatted.append(
                f"- {species['comName']} ({species['sciName']}) [{species['speciesCode']}]"
            )
        return "\n".join(formatted)
    ...
    def post(self, shared, prep_res, exec_res):
        ...
        if total_input == 0:
            ...
            logger.info(
                f"Species validation completed with {success_rate:.1%} success rate"
            )

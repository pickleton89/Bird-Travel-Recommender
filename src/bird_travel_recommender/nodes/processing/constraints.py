...
class FilterConstraintsNode(Node):
    ...
    def __init__(self):
        ...
        from ...utils.geo_utils import (
            ...
            is_within_radius,
            is_within_region,
            calculate_travel_time_estimate,
        )
    ...
    def prep(self, shared):
        ...
        if not all_sightings:
            ...
            logger.warning(
                "No sightings found in shared store - will process empty list"
            )

        return {"all_sightings": all_sightings, "constraints": constraints}
    ...
    def exec(self, prep_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        max_travel_radius_km = constraints.get(
            "max_travel_radius_km", max_daily_distance_km
        )
        ...
        min_observation_quality = constraints.get(
        ...
        if not all_sightings:
            ...
            return {"enriched_sightings": [], "filtering_stats": filtering_stats}
            ...
            if has_valid_gps:
                ...
                if (
                    ...
                    enriched_sighting["estimated_travel_time_hours"] = (
                        self.calculate_travel_time_estimate(distance_from_start)
                    )
                else:
                    enriched_sighting["distance_from_start_km"] = None
                    enriched_sighting["within_travel_radius"] = (
                        True  # No start location = no distance constraint
                    )
        ...
        if total_sightings > 0:
            filtering_stats["constraint_compliance_summary"] = {
                "valid_coordinates_pct": (
                    filtering_stats["valid_coordinates"] / total_sightings
                )
                * 100,
                "within_travel_radius_pct": (
                    filtering_stats["within_travel_radius"] / total_sightings
                )
                * 100,
                "within_date_range_pct": (
                    filtering_stats["within_date_range"] / total_sightings
                )
                * 100,
                "high_quality_pct": (
                    filtering_stats["high_quality_observations"] / total_sightings
                )
                * 100,
                "duplicate_rate_pct": (
                    filtering_stats["duplicates_flagged"] / total_sightings
                )
                * 100,
                "travel_feasible_pct": (
                    filtering_stats["travel_feasible"] / total_sightings
                )
                * 100,
                "fully_compliant_count": sum(
                    1
                    for s in enriched_sightings
                    if s.get("meets_all_constraints", False)
                ),
            }
    ...
    def post(self, shared, prep_res, exec_res):
        ...
        fully_compliant = exec_res["filtering_stats"][
            "constraint_compliance_summary"
        ].get("fully_compliant_count", 0)
        ...
        if fully_compliant == 0:
            logger.warning(
                "No sightings meet all constraints - may need to relax filters"
            )
        ...
        logger.info(
            f"Constraint filtering successful: {compliance_rate:.1%} compliance rate"
        )
        return "default"

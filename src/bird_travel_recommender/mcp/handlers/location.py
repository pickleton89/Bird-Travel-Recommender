...
class LocationHandlers:
    ...
    async def handle_get_region_details(
        self, region_code: str, name_format: str = "detailed", session=None
    ):
        """Handle get_region_details tool with input validation"""
        try:
            # Additional validation for name_format
            name_format = InputValidator.validate_string_length(
                name_format, "name_format", 20
            )
            ...
            region_info = self.ebird_api.get_region_info(
                region_code=region_code, name_format=name_format
            )
    ...
    async def handle_find_nearest_species(
        ...
        try:
            logger.info(
                f"Finding nearest observations for {species_code} at {lat},{lng}"
            )
    ...
    async def handle_get_nearby_species_observations(
        ...
        try:
            logger.info(
                f"Getting nearby observations for species {species_code} at {lat},{lng}"
            )
    ...
    async def handle_get_regional_statistics(
        self, region: str, days_back: int = 30, locale: str = "en"
    ):
        ...
        try:
            ...
            statistics = self.ebird_api.get_regional_statistics(
                region=region, days_back=days_back, locale=locale
            )

            return {
                "success": True,
                "region": region,
                "search_parameters": {"days_back": days_back, "locale": locale},
    ...
    async def handle_get_location_species_list(
        self, location_id: str, locale: str = "en"
    ):
        ...
        try:
            ...
            species_list = self.ebird_api.get_location_species_list(
                location_id=location_id, locale=locale
            )
    ...
    async def handle_get_subregions(
        ...
        try:
            ...
            subregions = self.ebird_api.get_subregions(
                region_code=region_code, region_type=region_type
            )
    ...
    async def handle_get_adjacent_regions(self, region_code: str):
    ...
    async def handle_get_elevation_data(
        ...
        try:
            logger.info(
                f"Getting elevation data for ({latitude}, {longitude}) within {radius_km}km"
            )

...
class CommunityHandlers:
    ...
    async def handle_get_recent_checklists(
        ...
        try:
            logger.info(
                f"Getting recent checklists for {region_code} (last {days_back} days)"
            )

            checklists = self.ebird_api.get_recent_checklists(
                region_code=region_code, days_back=days_back, max_results=max_results
            )
    ...
    async def handle_get_checklist_details(self, checklist_id: str):
    ...
    async def handle_get_user_stats(
        ...
        try:
            ...
            user_stats = self.ebird_api.get_user_stats(
                username=username, region_code=region_code, year=year
            )

            return {
                "success": True,
                "username": username,
                "search_parameters": {"region": region_code, "year": year},

...
from ...nodes import GenerateItineraryNode
    ...
    async def handle_generate_itinerary(
        ...
        **kwargs,
        ...
        try:
            ...
            return {"success": False, "error": str(e), "itinerary": ""}

    async def handle_plan_complete_trip(
        self,
        handlers_container,
        target_species: List[str],
        regions: List[str],
        trip_duration_days: int,
        max_daily_driving_hours: float = 4,
        ...
        try:
            logger.info(
                f"Planning complete trip for {len(target_species)} species in {regions}"
            )
            ...
            try:
                ...
                if region_info_result["success"]:
                    ...
                    region_display_name = region_info_result["region_info"].get(
                        "name", primary_region
                    )
                    ...
                    logger.warning(
                        f"Could not get region info for {primary_region}, using code as display name"
                    )
            except Exception as e:
                region_display_name = primary_region
            ...
            try:
                ...
                if regional_species_result["success"]:
                    ...
                    logger.info(
                        f"Found {len(regional_species_codes)} species ever reported in {region_display_name}"
                    )
                else:
            ...
            validate_result = (
                await handlers_container.species_handlers.handle_validate_species(
                    target_species
                )
            )
            ...
            validated_species_codes = [
                species["species_code"] for species in validated_species
            ]
            fetch_result = (
                await handlers_container.pipeline_handlers.handle_fetch_sightings(
                    validated_species_codes, regions, days_back
                )
            )
            ...
            filter_result = (
                await handlers_container.pipeline_handlers.handle_filter_constraints(
                    sightings, start_location, max_distance_km
                )
            )
            ...
            cluster_result = (
                await handlers_container.pipeline_handlers.handle_cluster_hotspots(
                    filtered_sightings, primary_region
                )
            )
            ...
            for cluster in hotspot_clusters:
                ...
                for hotspot in cluster.get("hotspots", []):
                    ...
                    if location_id:
                        try:
                            hotspot_detail_result = await handlers_container.location_handlers.handle_get_hotspot_details(
                                location_id
                            )
                                ...
                                hotspot_details.append(
                                    {
                                        ...
                                        "basic_info": hotspot,
                                        "detailed_info": hotspot_detail_result[
                                            "hotspot_info"
                                        ],
                                    }
                                )
                            ...
                            logger.warning(
                                f"Could not get details for hotspot {location_id}: {e}"
                            )
                            hotspot_details.append(
                                {
                                    "location_id": location_id,
            ...
            pipeline_results["enriched_hotspot_clustering"] = {
                ...
                "enrichment_count": len(
                    [
                        ...
                        for h in c.get("enriched_hotspots", [])
                    ]
                ),
            }
            logger.info(
                f"Enriched {len(enriched_clusters)} hotspot clusters with detailed information"
            )

            # Step 5: Score Locations
            score_result = (
                await handlers_container.pipeline_handlers.handle_score_locations(
                    hotspot_clusters, target_species
                )
            )
            ...
            route_result = (
                await handlers_container.pipeline_handlers.handle_optimize_route(
                    scored_locations, start_location
                )
            )
            ...
            itinerary_result = await self.handle_generate_itinerary(
                optimized_route, target_species, trip_duration_days
            )
                    ...
                    try:
                        ...
                        if (
                            ...
                            species_finding_recommendations.append(
                                {
                                    "species": species,
                                    "nearest_observations": nearest_obs_result[
                                        "observations"
                                    ][:3],  # Top 3 closest
                                    "total_nearby": len(
                                        nearest_obs_result["observations"]
                                    ),
                                }
                            )
                    except Exception as e:
                        logger.warning(
                            f"Could not get nearest observations for {species_code}: {e}"
                        )
            ...
            logger.info(
                f"Generated targeted finding recommendations for {len(species_finding_recommendations)} species"
            )
            ...
            return {
                ...
                "summary": {
                    ...
                    "species_with_targeted_recommendations": len(
                        species_finding_recommendations
                    ),
                    "route_distance_km": optimized_route.get("total_distance_km", 0),
                    "estimated_travel_time": optimized_route.get(
            ...
            return {"success": False, "error": str(e), "stage": "pipeline_execution"}

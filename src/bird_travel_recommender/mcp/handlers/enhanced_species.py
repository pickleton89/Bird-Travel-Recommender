...
class EnhancedSpeciesHandlers:
    ...
    async def handle_validate_species(self, species_names: List[str], **kwargs) -> Dict[str, Any]:
        """Enhanced validate_species handler with comprehensive error handling"""
        logger.info(f"Validating {len(species_names)} species with enhanced error handling")
        
        # Additional input validation
        if not species_names:
            raise ValidationError("Species list cannot be empty", "species_names", species_names)
        ...
        if not valid_species_names:
            raise ValidationError("No valid species names found after filtering", "species_names", species_names)
        ...
        try:
            ...
            if "taxonomy" in str(e).lower() or "species" in str(e).lower():
                raise APIError(f"Species validation failed: {str(e)}", endpoint="taxonomy")
            else:
    ...
    @with_circuit_breaker()
    @with_timeout(30)
    async def handle_get_regional_species_list(self, region: str, **kwargs) -> Dict[str, Any]:
        """Enhanced regional species list handler with error handling and retry logic"""
        logger.info(f"Getting species list for region {region} with enhanced error handling")
        
        # Additional region code validation
        if not region or not isinstance(region, str):
            raise ValidationError("Region code must be a non-empty string", "region", region)
        
        # Basic region code format validation
        region = region.strip().upper()
        if len(region) < 2 or len(region) > 10:
            raise ValidationError("Region code length must be between 2 and 10 characters", "region", region)
        ...
        try:
            ...
            return {
                ...
                "data_quality": {
                    "has_species_codes": any(isinstance(s, dict) and "speciesCode" in s for s in species_list[:5]),
    ...
    async def validate_multiple_regions(self, regions: List[str]) -> Dict[str, Any]:
        ...
        if not regions or not isinstance(regions, list):
            raise ValidationError("Regions must be a non-empty list", "regions", regions)
        
        if len(regions) > 10:  # Reasonable limit for batch processing
            raise ValidationError("Too many regions requested (max 10)", "regions", regions)

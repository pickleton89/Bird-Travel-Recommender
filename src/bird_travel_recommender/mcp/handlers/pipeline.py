...
class PipelineHandlers:
    ...
    async def handle_fetch_sightings(
        self, species_codes: List[str], regions: List[str], days_back: int = 30
    ):
        """Handle fetch_sightings tool"""
        try:
            logger.info(
                f"Fetching sightings for {len(species_codes)} species in {regions}"
            )
            ...
            shared_store = {
                ...
                "input": {"constraints": {"regions": regions, "days_back": days_back}},
            }

            # Execute FetchSightingsNode (BatchNode) with correct pattern
            prep_res = self.fetch_sightings_node.prep(
                shared_store
            ...
            return {"success": False, "error": str(e), "sightings": []}

    async def handle_filter_constraints(
        self,
        sightings: List[Dict],
        ...
        **kwargs,
        ...
        try:
            ...
            return {"success": False, "error": str(e), "filtered_sightings": []}

    async def handle_cluster_hotspots(
        self,
        filtered_sightings: List[Dict],
        region: str,
        cluster_radius_km: float = 15,
        **kwargs,
    ):
        ...
        try:
            ...
            return {"success": False, "error": str(e), "hotspot_clusters": []}

    async def handle_score_locations(
        self,
        hotspot_clusters: List[Dict],
        ...
        **kwargs,
        ...
        try:
            ...
            return {"success": False, "error": str(e), "scored_locations": []}

    async def handle_optimize_route(
        self,
        ...
        **kwargs,
        ...
        try:
            ...
            return {"success": False, "error": str(e), "optimized_route": {}}
    ...
    async def handle_get_historic_observations(
        ...
        try:
            logger.info(
                f"Getting historical observations for {region} on {year}-{month:02d}-{day:02d}"
            )
            ...
            return {
                ...
                "species_code": species_code,
                "search_parameters": {"locale": locale, "max_results": max_results},
    ...
    async def handle_get_yearly_comparisons(
        ...
        try:
            logger.info(
                f"Generating yearly comparisons for {region} on {reference_date}"
            )
            ...
            return {
                ...
                "species_code": species_code,
                "search_parameters": {"locale": locale},
    ...
    async def handle_get_migration_data(
        ...
        try:
            ...
            migration_data = self.ebird_api.get_migration_data(
                species_code=species_code, region_code=region_code, months=months
            )
            ...
            return {
                ...
                "species_code": species_code,
                "region": region_code,
                "search_parameters": {"months": months or list(range(1, 13))},
    ...
    async def handle_get_peak_times(
        self, species_code: str, latitude: float, longitude: float, radius_km: int = 25
    ):
        """Handle get_peak_times tool"""
        try:
            logger.info(
                f"Getting peak times for {species_code} at ({latitude}, {longitude})"
            )
    ...
    async def handle_get_seasonal_hotspots(
        self, region_code: str, season: str = "spring", max_results: int = 20
    ):
        ...
        try:
            ...
            seasonal_hotspots = self.ebird_api.get_seasonal_hotspots(
                region_code=region_code, season=season, max_results=max_results
            )
            ...
            return {
                ...
                "region": region_code,
                "season": season,
                "search_parameters": {"max_results": max_results},

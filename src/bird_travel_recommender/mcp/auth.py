...
class AuthManager:
    ...
    def authenticate_request(self, api_key: str) -> Optional[UserSession]:
        ...
        for stored_key in self.api_keys.values():
            if stored_key.key_hash == key_hash and stored_key.is_active:
                ...
                if (
                    session.is_locked
                    and session.lockout_until
                    and datetime.now() < session.lockout_until
                ):
                    logger.warning(
                        f"User {stored_key.user_id} is locked out until {session.lockout_until}"
                    )
                    return None
    ...
    def check_rate_limit(self, session: UserSession) -> bool:
        ...
        api_key = next(
            (k for k in self.api_keys.values() if k.user_id == user_id), None
        )
        if not api_key:
            return False
        ...
        self.rate_limits[user_id] = [
            ts for ts in self.rate_limits[user_id] if ts > one_hour_ago
        ]

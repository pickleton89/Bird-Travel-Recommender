...
class InputValidator:
    ...
    def validate_coordinates(lat: float, lng: float) -> None:
        ...
        if not isinstance(lat, (int, float)):
            raise ValidationError("lat", lat, "Latitude must be a number", "INVALID_TYPE")
        if not isinstance(lng, (int, float)):
            raise ValidationError("lng", lng, "Longitude must be a number", "INVALID_TYPE")
        
        if not -90 <= lat <= 90:
            raise ValidationError("lat", lat, f"Latitude must be between -90 and 90, got {lat}", "OUT_OF_RANGE")
        if not -180 <= lng <= 180:
            raise ValidationError("lng", lng, f"Longitude must be between -180 and 180, got {lng}", "OUT_OF_RANGE")
    
    @staticmethod
    def validate_region_code(code: str) -> str:
        """Validate and sanitize region code"""
        if not isinstance(code, str):
            raise ValidationError("region_code", code, "Region code must be a string", "INVALID_TYPE")
        ...
        if not REGION_CODE_PATTERN.match(code):
            raise ValidationError("region_code", code, 
    ...
    def validate_species_code(code: str) -> str:
        ...
        if not isinstance(code, str):
            raise ValidationError("species_code", code, "Species code must be a string", "INVALID_TYPE")
        
        code = code.strip().lower()
        if not SPECIES_CODE_PATTERN.match(code):
            raise ValidationError("species_code", code, 
    ...
    def validate_location_id(location_id: str) -> str:
        ...
        if not isinstance(location_id, str):
            raise ValidationError("location_id", location_id, "Location ID must be a string", "INVALID_TYPE")
        
        location_id = location_id.strip()
        if not LOCATION_ID_PATTERN.match(location_id):
            raise ValidationError("location_id", location_id, 
    ...
    def validate_string_length(value: str, field_name: str, max_length: int = MAX_STRING_LENGTH) -> str:
        ...
        if not isinstance(value, str):
            raise ValidationError(field_name, value, f"{field_name} must be a string", "INVALID_TYPE")
    ...
    @staticmethod
    def validate_array_size(arr: List[Any], field_name: str, max_size: int) -> List[Any]:
        """Validate array size to prevent DoS attacks"""
        if not isinstance(arr, list):
            raise ValidationError(field_name, arr, f"{field_name} must be a list", "INVALID_TYPE")
        
        if len(arr) > max_size:
            raise ValidationError(field_name, arr, 
                                f"{field_name} exceeds maximum size of {max_size} items", 
                                "TOO_MANY_ITEMS")
        
        if len(arr) == 0:
            raise ValidationError(field_name, arr, f"{field_name} cannot be empty", "EMPTY_LIST")
    ...
    def validate_numeric_range(value: Union[int, float], field_name: str, 
        ...
        if not isinstance(value, (int, float)):
            raise ValidationError(field_name, value, f"{field_name} must be a number", "INVALID_TYPE")
    ...
    def validate_date_string(date_str: str, field_name: str) -> str:
        ...
        if not isinstance(date_str, str):
            raise ValidationError(field_name, date_str, f"{field_name} must be a string", "INVALID_TYPE")
        
        if not DATE_PATTERN.match(date_str):
            raise ValidationError(field_name, date_str, 
...
COORDINATE_SCHEMA = {
    ...
    'lng': {
        'type': float,
        'validator': lambda x: InputValidator.validate_numeric_range(x, 'lng', -180, 180)
    }
}
...
SPECIES_LIST_SCHEMA = {
    'species_names': {
        'type': list,
        'validator': lambda x: InputValidator.validate_array_size(x, 'species_names', MAX_SPECIES_LIST_SIZE)
    }
}
...
DISTANCE_SCHEMA = {
    'distance_km': {
        'type': (int, float),
        'validator': lambda x: InputValidator.validate_numeric_range(x, 'distance_km', MIN_DISTANCE_KM, MAX_DISTANCE_KM)
    }
}
...
DAYS_BACK_SCHEMA = {
    'days_back': {
        'type': int,
        'validator': lambda x: InputValidator.validate_numeric_range(x, 'days_back', 1, MAX_DAYS_BACK)
    }
}
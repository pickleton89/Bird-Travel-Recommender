...
class AsyncFetchSightingsNode(AsyncNode):
        ...
        async with EBirdAsyncClient() as client:
            ...
            for species in species_list:
                task = self._create_species_task(
                    client, species, region_code, days_back, start_location, max_distance_km
                )
                tasks.append(task)
    ...
    async def _create_species_task(self, client: EBirdAsyncClient, species: Dict[str, Any], 
                                 region_code: str, days_back: int, start_location: Optional[Dict], 
                                 max_distance_km: int):
        """
        ...
        try:
            ...
            logger.debug(f"Async fetched {len(enriched_sightings)} sightings for {species['common_name']}")
    ...
    def _process_async_results(self, results: List, species_list: List[Dict]) -> Dict[str, Any]:
        """
    ...
    def post(self, shared, prep_res, exec_res):
        ...
        success_rate = stats["successful_fetches"] / stats["total_species"] if stats["total_species"] > 0 else 0
        ...
        logger.info(f"Async sightings fetch completed: {success_rate:.1%} success rate, "

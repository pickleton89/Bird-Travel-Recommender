...
from bird_travel_recommender.nodes import (
    ValidateSpeciesNode,
    FetchSightingsNode,
...
class TestPipelineIntegration:
    ...
    def test_end_to_end_pipeline_execution(
        self, pipeline_nodes, mock_ebird_api, integration_test_input
    ):
    ...
    def test_pipeline_with_no_valid_species(self, pipeline_nodes, mock_ebird_api):
        ...
        invalid_input = {
            ...
            "constraints": {"region": "US-MA", "days_back": 7},
        }
    ...
    def test_pipeline_error_propagation(self, pipeline_nodes):
        ...
        with patch(
            "bird_travel_recommender.utils.ebird_api.get_taxonomy"
        ) as mock_taxonomy:
    ...
    def test_pipeline_performance_with_large_dataset(
        self, pipeline_nodes, mock_ebird_api, large_species_dataset
    ):
        ...
        assert len(shared["validated_species"]) > 0
        print(
            f"Pipeline processed {len(large_species_dataset)} species in {duration:.2f} seconds"
        )
    ...
    def test_constraint_filtering_effectiveness(self, pipeline_nodes, mock_ebird_api):
        ...
        assert compliance_summary["fully_compliant_count"] <= len(
    ...
    @pytest.mark.integration
    @pytest.mark.mock
    def test_data_preservation_through_pipeline(
        self, pipeline_nodes, mock_ebird_api, integration_test_input
    ):

...
def test_filter_constraints_node():
    ...
    for i, scenario in enumerate(test_scenarios, 1):
        ...
        shared = {
            ...
            "input": {"constraints": scenario["constraints"]},
        }
        ...
        try:
            ...
            print(
                f"  High quality observations: {filtering_stats['high_quality_observations']}"
            )
            ...
            print("\nCompliance Summary:")
            print(
                f"  Fully compliant sightings: {compliance_summary['fully_compliant_count']}"
            )
            print(
                f"  Valid coordinates: {compliance_summary['valid_coordinates_pct']:.1f}%"
            )
            print(
                f"  Within travel radius: {compliance_summary['within_travel_radius_pct']:.1f}%"
            )
            print(
                f"  Within date range: {compliance_summary['within_date_range_pct']:.1f}%"
            )
            print(f"  High quality: {compliance_summary['high_quality_pct']:.1f}%")
            print(
                f"  Travel feasible: {compliance_summary['travel_feasible_pct']:.1f}%"
            )

            # Show enriched sighting examples
            print("\nEnriched Sighting Examples:")
            compliant_sightings = [
                s for s in enriched_sightings if s.get("meets_all_constraints", False)
            ]
            non_compliant_sightings = [
                s
                for s in enriched_sightings
                if not s.get("meets_all_constraints", False)
            ]
            ...
            if compliant_sightings:
                ...
                for sighting in compliant_sightings[:2]:  # Show first 2
                    ...
                    print(
                        f"      Distance: {sighting.get('distance_from_start_km', 'N/A')} km"
                    )
            ...
            if non_compliant_sightings:
                print(
                    f"  Non-compliant sightings ({len(non_compliant_sightings)}) - showing first 2:"
                )

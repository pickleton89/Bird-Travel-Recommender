        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            try:
                ...
                if analysis.parameters.species:
                    print(
                        f"Extracted Species: {', '.join(analysis.parameters.species)}"
                    )
                if analysis.parameters.locations:
                    print(
                        f"Extracted Locations: {', '.join(analysis.parameters.locations)}"
                    )
                if analysis.parameters.timeframes:
                    print(
                        f"Extracted Timeframes: {', '.join(analysis.parameters.timeframes)}"
                    )

                # Test parameter conversion
                tool_params = nlp.get_enhanced_parameters_for_tool(
                ...
                try:
                    ...
                    print(
                        f"Fallback Confidence: {fallback_analysis.confidence_score:.2f}"
                    )
    ...
    try:
        ...
        for i, request in enumerate(test_requests, 1):
            ...
            shared_store = {
                ...
                "context": {"default_coordinates": {"lat": 42.3601, "lng": -71.0589}},
            }
            ...
            try:
                ...
                if "tool_execution_plan" in shared_store:
                    ...
                    if hasattr(plan, "intent_analysis"):
                        print(
                            f"Primary intent: {plan.intent_analysis.primary_intent.value}"
                        )
...
def test_full_enhanced_agent():
    ...
    try:
        ...
        request = (
            "I want to plan a birding trip to see cardinals and warblers around Boston"
        )

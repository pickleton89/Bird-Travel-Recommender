...
def test_auth_manager():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        raw_key = auth_manager.create_api_key(
            "test_user", ["read:species", "get:advice"]
        )
...
def test_rate_limiter():
    ...
    test_config = RateLimitConfig(max_requests=3, window_seconds=60, burst_allowance=1)
    rate_limiter.set_user_limit(user_id, test_config)

    print(
        f"Set rate limit: {test_config.max_requests + test_config.burst_allowance} requests per {test_config.window_seconds}s"
    )

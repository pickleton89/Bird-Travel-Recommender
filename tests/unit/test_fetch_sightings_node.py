...
class TestFetchSightingsNode:
    ...
    @pytest.fixture
    def shared_store_with_validated_species(
        self, mock_validated_species, test_constraints
    ):
        """Shared store with validated species data."""
        return {
            "validated_species": mock_validated_species,
            "input": {"constraints": test_constraints},
        }
    ...
    @pytest.mark.unit
    @pytest.mark.mock
    def test_nearby_observations_endpoint_selection(
        self, fetch_node, mock_ebird_api, shared_store_with_validated_species
    ):
    ...
    def test_species_observations_endpoint_selection(self, fetch_node, mock_ebird_api):
        ...
        if shared["all_sightings"]:
            assert any(
                s.get("fetch_method") == "species_observations"
                for s in shared["all_sightings"]
            )
    ...
    def test_rate_limiting(self, fetch_node, mock_ebird_api):
        ...
        with patch("time.sleep") as mock_sleep:
            shared = {
                ...
                "input": {"constraints": {"region": "US-MA", "days_back": 7}},
            }
    ...
    def test_api_error_handling(self, fetch_node):
        ...
        with (
            patch(
                "bird_travel_recommender.utils.ebird_api.get_recent_observations"
            ) as mock_recent,
            patch(
                "bird_travel_recommender.utils.ebird_api.get_species_observations"
            ) as mock_species,
        ):
    ...
    @pytest.mark.unit
    @pytest.mark.mock
    def test_fetch_statistics_generation(
        self, fetch_node, mock_ebird_api, shared_store_with_validated_species
    ):

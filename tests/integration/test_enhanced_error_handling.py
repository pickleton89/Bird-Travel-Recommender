...
from src.bird_travel_recommender.mcp.handlers.enhanced_species import (
    EnhancedSpeciesHandlers,
)
...
class TestEnhancedErrorHandling:
    ...
    def enhanced_handler(self):
        ...
        with patch.dict(
            "os.environ", {"EBIRD_API_KEY": "test_key", "OPENAI_API_KEY": "test_key"}
        ):
        ...
        async def test_function(should_fail: bool = False):
            if should_fail:
                raise ValidationError(
                    "Test validation error", "test_field", "test_value"
                )
    ...
    async def test_enhanced_species_validation(self, enhanced_handler):
        ...
        with (
            patch.object(enhanced_handler.validate_species_node, "prep") as mock_prep,
    ...
    async def test_batch_processing_errors(self, enhanced_handler):
        ...
        with patch.object(
            enhanced_handler, "handle_get_regional_species_list"
        ) as mock_handler:
            ...
            result = await enhanced_handler.validate_multiple_regions(
                ["US-MA", "INVALID", "US-CA"]
            )

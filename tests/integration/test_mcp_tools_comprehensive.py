...
class TestMCPToolsComprehensive:
    ...
    def mcp_server(self):
        ...
        with patch.dict('os.environ', {'EBIRD_API_KEY': 'test_key_12345', 'OPENAI_API_KEY': 'test_openai_key'}):
    ...
    async def test_all_tools_registered(self, mcp_server):
        ...
        expected_location = [
            "get_region_details", "get_hotspot_details", "find_nearest_species",
            "get_nearby_notable_observations", "get_nearby_species_observations",
            "get_top_locations", "get_regional_statistics", "get_location_species_list",
            "get_subregions", "get_adjacent_regions", "get_elevation_data"
        ]
        expected_pipeline = [
            "fetch_sightings", "filter_constraints", "cluster_hotspots",
            "score_locations", "optimize_route", "get_historic_observations",
            "get_seasonal_trends", "get_yearly_comparisons", "get_migration_data",
            "get_peak_times", "get_seasonal_hotspots"
        ]
        expected_planning = ["generate_itinerary", "plan_complete_trip"]
        expected_advisory = ["get_birding_advice"]
        expected_community = ["get_recent_checklists", "get_checklist_details", "get_user_stats"]
        
        all_expected = (expected_species + expected_location + expected_pipeline + 
                       expected_planning + expected_advisory + expected_community)
    ...
    @patch('src.bird_travel_recommender.mcp.handlers.species.SpeciesHandlers.handle_validate_species')
    ...
    async def test_concurrent_tool_execution(self, mcp_server):
        ...
        with patch('src.bird_travel_recommender.mcp.handlers.species.SpeciesHandlers.handle_validate_species') as mock_species, \
             patch('src.bird_travel_recommender.mcp.handlers.location.LocationHandlers.handle_get_region_details') as mock_region:
            
            mock_species.return_value = {"success": True, "validated_species": []}
            mock_region.return_value = {"success": True, "region_info": {}}
            
            tasks = [
                mcp_server._route_tool_call("validate_species", {"species_names": ["Cardinal"]}),
